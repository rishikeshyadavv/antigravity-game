 ...
 *  
 *  WEB VERSION — compiled to WebAssembly via Emscripten
 *
 *  Build (Emscripten):
 *      emcc antigravity_web.c -o web/index.html -Os -Wall
 *           -DPLATFORM_WEB
...
static void UpdateFrame(void)
{
    ...
    if (state == STATE_MENU)
    {
        /* Accept ENTER, SPACE, or mouse click/tap to start */
    ...
    else if (state == STATE_PLAYING)
    {
        ...
        /* --- Gravity / antigravity (SPACE or mouse/touch) ----- */
        bool spaceHeld = IsKeyDown(KEY_SPACE) || IsMouseButtonDown(MOUSE_BUTTON_LEFT);
    ...
    {
        bool flipped = (IsKeyDown(KEY_SPACE) || IsMouseButtonDown(MOUSE_BUTTON_LEFT))
                       && (state == STATE_PLAYING);
    ...
    if (state == STATE_MENU)
    {
        ...
        int fs = 18;
        DrawText("SPACE / CLICK  Flip gravity upward",   boxX + 24, boxY +  56, fs, ruleCol);
        DrawText("Release        Normal gravity down",   boxX + 24, boxY +  80, fs, ruleCol);
        DrawText("P              Pause / Resume",        boxX + 24, boxY + 104, fs, ruleCol);

        DrawLine(boxX + 20, boxY + 134, boxX + boxW - 20, boxY + 134, (Color){100, 255, 180, 80});

        Color tipCol = (Color){255, 220, 100, 255};
        DrawText("Dodge the green pillars to score!",    boxX + 24, boxY + 148, fs, tipCol);
        DrawText("Hit a wall, ceiling, or floor = Game Over", boxX + 24, boxY + 172, fs, tipCol);

        /* Start prompt */
        const char *startText = "Press ENTER / SPACE / TAP to start";
    ...
    if (state == STATE_GAMEOVER)
    {
        ...
        const char *restartText = "Press ENTER / TAP to restart";
...
#ifdef __EMSCRIPTEN__
    emscripten_set_main_loop(UpdateFrame, TARGET_FPS, 1);
#else
